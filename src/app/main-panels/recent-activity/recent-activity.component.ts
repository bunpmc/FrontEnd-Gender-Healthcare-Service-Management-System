...
export class RecentActivityComponent implements OnInit {
  ...
  async loadRecentNotifications() {
    this.isLoading = true;
    try {
      const notifications = await this.supabaseService.getRecentNotifications();
      this.activities = notifications.map(notification => ({
  ...
  private getTitle(notification: any): string {
  ...
  private getDescription(notification: any): string {
